...
from dns_utils.utils import (
    async_recvfrom,
    async_sendto,
    build_socks5_target_payload,
    build_socks5_udp_tcp_frame,
    generate_random_hex_text,
    getLogger,
    parse_socks5_address_bytes,
    read_socks5_udp_tcp_frame,
)
...
class MasterDnsVPNClient(PacketQueueMixin):
    ...
    def __init__(self) -> None:
        ...
        self._control_request_ack_map = {
            ...
            Packet_Type.UDP_ASSOCIATE: Packet_Type.UDP_ASSOCIATE_ACK,
            Packet_Type.UDP_CLOSE: Packet_Type.UDP_CLOSE_ACK,
        }
        ...
        self._packable_control_types.update(
            {
                ...
                Packet_Type.UDP_ASSOCIATE,
                Packet_Type.UDP_CLOSE,
            }
        )
    ...
    @staticmethod
    def _build_socks5_success_reply() -> bytes:
        return b"\x05\x00\x00\x01\x00\x00\x00\x00\x00\x00"

    @staticmethod
    def _prune_udp_seen(stream_data: dict, key: str, now_mono: float) -> None:
        seen = stream_data.setdefault(key, {})
        expired = [seq for seq, ts in seen.items() if now_mono - ts > 180.0]
        for seq in expired:
            seen.pop(seq, None)

    def _mark_udp_seen(self, stream_data: dict, key: str, sn: int) -> bool:
        now_mono = time.monotonic()
        self._prune_udp_seen(stream_data, key, now_mono)
        seen = stream_data.setdefault(key, {})
        if sn in seen:
            return False
        seen[sn] = now_mono
    ...
    def _store_udp_fragment(
        ...
        bucket = stream_data.setdefault(bucket_key, {})
        entry = bucket.setdefault(
            sn,
            {
                "total": max(1, min(int(total_fragments or 1), 255)),
                "chunks": {},
                "time": time.monotonic(),
            },
        )
        entry["total"] = max(1, min(int(total_fragments or 1), 255))
        entry["time"] = time.monotonic()
        entry["chunks"][int(fragment_id)] = data or b""

        if len(entry["chunks"]) != entry["total"]:
            return None
        if any(i not in entry["chunks"] for i in range(entry["total"])):
            return None
    ...
    async def _handle_local_tcp_connection(self, reader, writer):
        ...
        socks_cmd = 0x01
        atyp = 0x01
        target_addr_bytes = b"\x00\x00\x00\x00"
        target_port_bytes = b"\x00\x00"
        ...
        if self.protocol_type == "SOCKS5":
            try:
                ...
                target_payload = build_socks5_target_payload(
                    socks_cmd,
                    bytes([atyp]) + target_addr_bytes + target_port_bytes,
                )
        ...
        if not is_socks5:
            ...
            try:
                ...
                if stream_data.get("status") == "ACTIVE":
                    if writer and not writer.is_closing():
                        if socks_cmd == 0x05:
                            reply = self._build_socks5_success_reply()
                        else:
