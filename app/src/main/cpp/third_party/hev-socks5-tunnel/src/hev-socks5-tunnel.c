                  ...
                  const ip_addr_t *addr, u16_t port)
{
    ...
    b->len = res;
    b->tot_len = res;
    /* Reply to the original request source as a synthetic DNS server. */
                  ...
                  const ip_addr_t *addr, u16_t port)
{
    ...
    if (dns && pcb->local_ip.type == IPADDR_TYPE_V4) {
        int faddr = hev_config_get_mapdns_address ();
        int fport = hev_config_get_mapdns_port ();
        /* Match configured mapped-DNS destination (not packet source). */
        if (fport == pcb->local_port &&
            faddr == ip_2_ip4 (&pcb->local_ip)->addr) {
            udp_recv (pcb, dns_recv_handler, dns);
            dns_recv_handler (dns, pcb, p, addr, port);
            return;
        }
    }
